"""Run RNN and DNN in Labeled and Click data"""
...
def operate_model_paras(sess, is_pair, paras, is_bias, is_truth):
    assert isinstance(paras, dict)
    return RNN(sess=sess, feature_space=paras['feature_space'], l2_weight=paras['l2_weight'],
               ...
               tf_device=paras['tf_device'], is_bias=is_bias, is_truth=is_truth)


def running_example(algo, running_round, is_pair, training_round, model_paras, is_bias, is_truth):
    """run run_data.py for data generation before run run_bias.py"""
    # initial the model
    config = tf.ConfigProto(log_device_placement=False)
    config.gpu_options.allow_growth = True
    sess = tf.Session(config=config)
    model = operate_model_paras(sess=sess, is_pair=is_pair, paras=model_paras, is_bias=is_bias, is_truth=is_truth)
    ...
    print_type_str = '---------------- YOU ARE IN BIAS SETTING ----------------------'
    print(print_type_str)
    print("==================== START LOADING =================")
    train_batch_replay_buffer, test_batch_replay_buffer, train_batch_pair_replay_buffer, test_batch_pair_replay_buffer = load_replay_buffer(data_dir=data_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--is_pair', type=bool, help='Switch for pair-wise', default=False)
    ...
    parser.add_argument('-b', '--is_bias', type=bool, help='Switch for bias', default=False)
    parser.add_argument('-i', '--is_truth', type=bool, help='Switch for truth', default=True)
    args = parser.parse_args()

    running_example(args.algo, args.running_round, args.is_pair, args.training_round, args.model_paras, args.is_bias, args.is_truth)
